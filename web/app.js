...
const TIPS = {
  grid_levels:'Сколько уровней в лестнице с КАЖДОЙ стороны от центра. При 10 сетка держит около 20 заявок: 10 покупок ниже цены и 10 продаж выше. Больше уровней = лестница накрывает более широкий диапазон, но каждая заявка мельче (аллокация делится на всех), и на дорогих инструментах она может не дотянуть до минимального лота биржи — тогда пара отключается с пояснением.',
  grid_notional_mult:'Каким объёмом сетка реально торгует, в долях выданного ей капитала. ×1 — суммарный размер лестницы равен аллокации, это работа фактически без плеча. ×3 при плече 3× — весь доступный лимит выбран: прибыль с одного прохода уровня втрое больше, но и цена ликвидации втрое ближе. Выше плеча поднять нельзя — потолок обрежет. Именно этот параметр, а не плечо, отвечает за то, доллары у вас на сделке или центы.',
  grid_reanchor:'Перестановка всей лестницы, если цена ушла дальше её края на указанное число шагов. 0 (по умолчанию) = выключено: уровни стоят НЕПОДВИЖНО, как в классическом гриде. Включать стоит с осторожностью — перестановка в тренде превращает сетку в погоню за ценой, ровно то, из-за чего маркет-мейкинговый режим давал минус во всех окнах.',
...
function qq(key){
  // Знак вопроса без текста — мёртвый элемент: человек наводит мышь и ничего
  // не получает. Лучше не рисовать его вовсе, тогда пропуск сразу заметен.
  return TIPS[key] ? `<span class="qq" data-tip="${TIPS[key]}">?</span>` : '';
}
...
function lossCard(d){
  ...
  let tip='Изменение счёта с начала: эквити минус внесённый депозит. '+
    'На этом счёте убыток БЕЗ ДНА: он не упирается в размер депозита, а показывает полную '+
    'накопленную цифру — то, что на бирже пришлось бы покрывать доливками. '+
    'На реальном счёте при изолированной марже глубже внесённого потерять нельзя, '+
    'поэтому бэктест и опубликованные результаты считаются с дном.';
  if(liq) tip += ` Выносов по марже за сессию: ${liq}, сгорело $${(d.liq_burned||0).toFixed(2)}. `+
    'Каждый закрывает позицию, но торговлю не останавливает.';
  return card('ИТОГ СЧЁТА', (d.pnl_total>=0?'+':'')+fmt(d.pnl_total).replace('-',''),
    d.pnl_total>=0?C.green:C.red, tip)
    + (liq? card('ВЫНОСОВ', String(liq), C.red,
        `Сгорело $${(d.liq_burned||0).toFixed(2)}. Каждый вынос закрывает позицию по марже, но торговля продолжается.`) : '');
}
...
function setupTooltips(){
  ...
  document.addEventListener('mouseover',e=>{const t=e.target.closest('[data-tip]'); if(t){ S._tipT=t;
    // 450 мс ощущались как «подсказки нет»: человек наводит на знак вопроса
    // и почти полсекунды смотрит в пустоту. Для явного «?» это слишком долго.
    clearTimeout(S._tipTimer); S._tipTimer=setTimeout(()=>{ if(S._tipT===t) show(t); }, 200); }});
