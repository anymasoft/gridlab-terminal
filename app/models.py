...
class GridParams(BaseModel):
    ...
    # Плечо КОНКРЕТНОЙ сетки — как на бирже, где оно выставляется на инструмент.
    # 0 = взять из конфига (.env LEVERAGE). Реальный предел свой у каждого
    # инструмента (BTC 100x, BNB 50x, DOGE 75x) — приходит из instruments-info
    # и обрезает это число.
    #
    # Плечо НЕ задаёт размер заявки — на бирже оно определяет только то, какая
    # доля позиции замораживается под обеспечение и где стоит цена ликвидации.
    # Размером сетки управляет grid_notional_mult, а плечо задаёт его потолок.
    leverage: float = 0.0
